

package university.management.system1;
import javax.swing.*;
import java.awt.*;
import java.sql.*;
import java.awt.event.*;//ActionListener
public class EnterMarks extends JFrame implements ActionListener {
    Choice crollno;//DropDwon Choice and JCombox se ban sakta hai dono se
    JComboBox cbsemester;//DropDwon Choice and JCombox se ban sakta hai dono se
    JTextField tfsub1,tfsub2,tfsub3,tfsub4,tfsub5,tfmarks1,tfmarks2,tfmarks3,tfmarks4,tfmarks5;
    JButton submit,cancel;
    EnterMarks()
    {
        setSize(1000,500);
        setLocation(250,150);
        setLayout(null);
        
        getContentPane().setBackground(Color.WHITE);
        //Photo inserting
        ImageIcon i1=new ImageIcon(ClassLoader.getSystemResource("icons/exam.jpg"));
        Image i2=i1.getImage().getScaledInstance(400,300,Image.SCALE_DEFAULT);
        ImageIcon i3=new ImageIcon(i2);
        JLabel image=new JLabel(i3);
        image.setBounds(350,0,200,200);
        image.setBounds(500,40,400,300);
        add(image);
        
        JLabel heading=new JLabel("Enter Marks of Students");
        heading.setBounds(50, 0, 500, 50);
        heading.setFont(new Font("serif",Font.ITALIC,35));
        add(heading);
         
        JLabel lblrollnumber=new JLabel("Select Roll Number");
        lblrollnumber.setBounds(50,70,150,20);//x=left to right move and y=up to down move 
        add(lblrollnumber);//yeh add print karwata hai value ko frame k upar
        //Class for the Drop down creating 
        crollno=new Choice();
        crollno.setBounds(200,70,150,20);
        add(crollno);
        //forn the exception handling
        try{
            Conn c=new Conn();
           ResultSet rs= c.s.executeQuery("select *  from student");
           while(rs.next())
           {
               crollno.add(rs.getString("rollno"));
           }
        }catch(Exception e)
        {
            e.printStackTrace();
        }
        
        JLabel lblsemester=new JLabel("Semester");
        lblsemester.setBounds(50,110,150,20);//x=left to right move and y=up to down move 
        add(lblsemester);
        
        
        String semester[]={"1st Semester","2nd Semester","3rd Semester","4th Semester","5th Semester","6th Semester","7th Semester","8th Semester"};
        
        cbsemester=new JComboBox(semester);
        cbsemester.setBounds(200, 110, 150,20);
        cbsemester.setBackground(Color.WHITE);
        add(cbsemester);
        
         JLabel lblenetrsubject=new JLabel("Enter Subjects");
        lblenetrsubject.setBounds(100,150,200,40);//x=left to right move and y=up to down move 
        add(lblenetrsubject);
        
         JLabel lblentermark=new JLabel("Enter Marks");
        lblentermark.setBounds(320,150,200,40);//x=left to right move and y=up to down move 
        add(lblentermark);
        //1textfieldssub
        tfsub1=new JTextField();
        tfsub1.setBounds(50, 200, 200, 20);
        add(tfsub1);
        
        //1textfieldssub
        tfsub2=new JTextField();
        tfsub2.setBounds(50, 230, 200, 20);
        add(tfsub2);
        //1textfieldssub
        tfsub3=new JTextField();
        tfsub3.setBounds(50, 260, 200, 20);
        add(tfsub3);
        //1textfieldssub
        tfsub4=new JTextField();
        tfsub4.setBounds(50, 290, 200, 20);
        add(tfsub4);
        //1textfieldssub
        tfsub5=new JTextField();
         tfsub5.setBounds(50, 320, 200, 20);
        add( tfsub5);
        
        //1textfieldMarks
        tfmarks1=new JTextField();
        tfmarks1.setBounds(270, 200, 200, 20);
        add(tfmarks1);
        
        //1textfieldMarks
        tfmarks2=new JTextField();
        tfmarks2.setBounds(270, 230, 200, 20);
        add(tfmarks2);
        //1textfieldMarks
        tfmarks3=new JTextField();
        tfmarks3.setBounds(270, 260, 200, 20);
        add(tfmarks3);
        //1textfieldMarks
        tfmarks4=new JTextField();
         tfmarks4.setBounds(270, 290, 200, 20);
        add( tfmarks4);
        //1textfieldMarks
         tfmarks5=new JTextField();
         tfmarks5.setBounds(270, 320, 200, 20);
        add( tfmarks5);
        
        //Button for Submit
         submit=new JButton("Submit");
        submit.setBounds(70, 360, 150, 25);
        submit.setBackground(Color.BLACK);
        submit.setForeground(Color.WHITE);
        submit.addActionListener(this); //Button click
       // login.setFont(new Font("Tahoma ",Font.BOLD,16));
        add(submit);
        
          //Button for cancel
         cancel=new JButton("Back");
         cancel.setBounds(300, 360, 150, 25);
         cancel.setBackground(Color.BLACK);
         cancel.setForeground(Color.WHITE);
         cancel.addActionListener(this); //Button click
       // login.setFont(new Font("Tahoma ",Font.BOLD,16));
        add(cancel);
        
        setVisible(true);
    }
    //Use Unimplemented abstract method use for ActionListner to use in Submit And Back Butoon
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getSource()==submit)
        {
            //for data save use and error handling 
            try{
                Conn c=new Conn();
                //Inserting into mysql
                String query1="insert into subject values('"+crollno.getSelectedItem()+"','"+cbsemester.getSelectedItem()+"','"+tfsub1.getText()+"','"+tfsub2.getText()+"','"+tfsub3.getText()+"','"+tfsub4.getText()+"','"+tfsub5.getText()+"')";
                String query2="insert into marks values('"+crollno.getSelectedItem()+"','"+cbsemester.getSelectedItem()+"','"+tfmarks1.getText()+"','"+tfmarks2.getText()+"','"+tfmarks3.getText()+"','"+tfmarks4.getText()+"','"+tfmarks5.getText()+"')";
                
             //Both query are executed in below process
             c.s.executeUpdate(query1);
             c.s.executeUpdate(query2);
             //For pup up after submitted showing 
             JOptionPane.showMessageDialog(null, "Marks Inserted Successfully");
             setVisible(false);
            }catch(Exception e)
            {
                e.printStackTrace();//for exception handling 
            }
        }else
        {
            setVisible(false);
        }
    }
  
    
    public static void main(String[]args)
    {
        new EnterMarks();
    }
    
}
